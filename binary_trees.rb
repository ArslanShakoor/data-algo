...
class Node
  ...
  def add(val)
    if val < @value
      @left.nil? ? @left = Node.new(val) : @left.add(val)
    else
      @right.nil? ? @right = Node.new(val) : @right.add(val)
    end
  end      
end
...
p binarySearch.add(10)
p binarySearch.add(15)
p binarySearch.add(8)
p binarySearch.add(25)
p binarySearch.inspect